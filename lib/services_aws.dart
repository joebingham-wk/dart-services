...
final Logger _logger = Logger('aws_server');

void main(List<String> args) {
  int awsPort = 8080;
  if (args.isNotEmpty) awsPort = int.parse(args[0]);
  ...
  log.info('''Initializing dart-services:
    ...
    REDIS_VERSION: ${io.Platform.environment['REDIS_VERSION']}
  ''');

  AwsServer server =
      AwsServer(sdk, io.Platform.environment['REDIS_SERVER_URI']);
  server.start(awsPort);
}
...
class AwsServer {
  ...
  AwsServer(this.sdkPath, this.redisServerUri) {
    ...
    commonServer = CommonServer(
        ...
        redisServerUri == null
            ? InMemoryCache()
            : RedisCache(
                redisServerUri, io.Platform.environment['REDIS_VERSION']));
...
class AwsServerContainer implements ServerContainer {
  @override
